...
import ImageBrowseByDress from "../../components/ImageBrowseByDress";
const BrowseByDress = () => {
  return (
    <div className="m-auto mx-4 flex max-w-[1239px] flex-col justify-center gap-7 bg-secondary px-6 sm:gap-16 md:px-[64px] md:py-[70px]">
      ...
      <div className="flex flex-col gap-5 md:grid md:grid-cols-3 md:grid-rows-2">
        {DataBrowseBy.map((item, index) => (
          <ImageBrowseByDress key={index} img={item.img} col={item.col} name={item.name}/>
        ))}
      </div>
    </div>
  );
